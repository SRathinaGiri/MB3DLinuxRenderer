...
uses
  Classes, TypeDefinitions, Math3D;

type
  TMB3DRenderOptions = record
    ThreadCount: Integer;
    ThreadPriority: TThreadPriority;
    CalculateHardShadows: Boolean;
  end;

function DefaultMB3DRenderOptions: TMB3DRenderOptions;
function MakeMB3DRenderOptions(ThreadCount: Integer;
  ThreadPriority: TThreadPriority): TMB3DRenderOptions;
function CalcMandT(Header: TPMandHeader10; PLightVals: TPLightVals; PCTS: TPCalcThreadStats;
                   PsiLight5: TPsiLight5; hSLoffset, FSIstart, FSIoffset: Integer; hRect: TRect;
                   const RenderOptions: TMB3DRenderOptions): Boolean;
...
var CalcMaps: array[0..2] of TLightMap; //max 3 lightmaps?   todo: Global Map Handler with depencies + exclusive free+load
    ...
    CalcLastError: string;
  //  CalcDE: TCaldDEfunction = CalcDEfull;

implementation

uses CalcThread, CalcThread2D, HeaderTrafos, Math,
     {$IFDEF MB3D_HEADLESS}MB3DHeadlessMaps, MB3DHeadlessUtils,
     {$ELSE}MB3DMaps, DivUtils, ImageProcess, FileHandling, MapSequences,{$ENDIF}
     SysUtils, Types
     {$IFDEF WINDOWS}, Windows{$IFNDEF MB3D_HEADLESS}, Forms{$ENDIF}{$ENDIF};
...
var i: Integer;
{$IFNDEF MB3D_HEADLESS}
    i2, j, n, c, Mcount: Integer;
    bIsIpol: LongBool;
    MA: array[0..2] of Integer;
{$ENDIF}
begin
{$IFDEF MB3D_HEADLESS}
    for i := 0 to 2 do FreeLightMap(@CalcMaps[i]);
{$ELSE}
    Mcount := 0;
    ...
    begin
      j := 0;
      c := 0;
      while j < Mcount do // delete already loaded MB3DMaps from list
      begin
  ...
  {  FreeLightMap(CalcMaps[0]); //no, mark MB3DMaps as notused, afterwards free only ehich are still notused!
    FreeLightMap(CalcMaps[1]);
    FreeLightMap(CalcMaps[2]);   }
{$ENDIF}
end;
...
begin
  ...
  Result.ThreadPriority := tpNormal;
  Result.CalculateHardShadows := False;
end;
...
begin
  ...
  Result.ThreadPriority := ThreadPriority;
  Result.CalculateHardShadows := False;
end;
...
begin
  CalcLastError := '';
  ThreadCount := Min(Max(1, RenderOptions.ThreadCount), Header.Height);
  try
    ...
    if Result then begin
      ...
      if RenderOptions.CalculateHardShadows then
  ...
  begin
    ...
    begin
      ...
      begin
        try
          ...
          MandCalcThread[x - 1].FreeOnTerminate := {$IFDEF MB3D_HEADLESS}False{$ELSE}True{$ENDIF};
          ...
          MandCalcThread[x - 1].Priority        := RenderOptions.ThreadPriority;
        ...
        try
          ...
          MandCalcThread2D[x - 1].FreeOnTerminate := {$IFDEF MB3D_HEADLESS}False{$ELSE}True{$ENDIF};
          {$ifdef PARAMS_PER_THREAD}
          OldVerbose := bGetMCTPverbose;
          try
            bGetMCTPverbose := False;
          ...
          MandCalcThread2D[x - 1].Priority        := RenderOptions.ThreadPriority;
          PCTS.CTrecords[x].isActive := 1;
    ...
    if MCTparas.calc3D then
      ...
      begin
        MandCalcThread[x].WaitFor;
        if MandCalcThread[x].FatalException <> nil then
        begin
          Result := False;
          if MandCalcThread[x].FatalException is Exception then
            CalcLastError := Exception(MandCalcThread[x].FatalException).Message
          else
            CalcLastError := 'Unhandled render-thread exception';
        end;
        MandCalcThread[x].Free;
      end
    else
      for x := 0 to ThreadCount - 1 do
      begin
        MandCalcThread2D[x].WaitFor;
        if MandCalcThread2D[x].FatalException <> nil then
        begin
          Result := False;
          if MandCalcThread2D[x].FatalException is Exception then
            CalcLastError := Exception(MandCalcThread2D[x].FatalException).Message
          else
            CalcLastError := 'Unhandled render-thread exception';
        end;
        MandCalcThread2D[x].Free;
      end;
    {$ENDIF}
  end;
end;
      ...
      begin
        ...
        begin
          ...
          if (Result >= msDEstop) then Dec(It3Dex.ItResultI); //else probs with decomb and rpow3-> no stop...
                                                            ...
                                                            //dsG negative?
procedure RMCalcRoughness(N: TPVec3D; var sRough: Single; dt2, dsG: PDouble);
var denom, num, val: Double;
begin
  // dT2 := Noffset * 0.5 / (dM * StepSNorm);
  denom := d1em100 + Sqr(N[0]) + Sqr(N[1]) + Sqr(N[2]);
  num := d1em100 + dsG^ * 7 * Sqr(dt2^);
  val := Sqrt(num / denom) - 0.05;
  sRough := Clamp01D(val);
end;
...
begin
    with pMCTparas^ do
    begin
      if MCTCameraOptic = 2 then
      begin
        pIt3Dext.C1 := Ystart[0];
        pIt3Dext.C2 := Ystart[1];
        pIt3Dext.C3 := Ystart[2];
      end
      else
      begin
...
procedure RMCalculateVgradsFOV(pMCTparas: PMCTparameter; ix: Integer);
begin
    case pMCTparas.MCTCameraOptic of
      1: CalculateVgradsFOVRect(pMCTparas, ix);
      2: CalculateVgradsFOVPanorama(pMCTparas, ix);
    else
      CalculateVgradsFOVCommon(pMCTparas, ix);
    end;
end;
    ...
    begin
      ...
      begin
        ...
        if pIt3Dext.ItResultI >= MaxItsResult then
          dT1 := -Abs(dT1) else
        begin
          if DE < msDEstop then dT1 := Abs(dT1) * -0.55
                           else dT1 := Abs(dT1) * 0.55;
        end;
      end;
      iMaxIt := itst;
...
var itmp, iHSnr, ShadowStepCount: Integer;
    ...
    begin
      ...
      begin
        ...
        repeat
          Inc(ShadowStepCount);
          RLastDE := dTmp;
          RLastStepWidth := MinCS(
            MaxCS(s011, (dTmp - msDEsub * msDEstop) * sZstepDiv *
              RStepFactorDiff),
        ...
        until (dT1 < 0) or (ShadowStepCount >= 4096);
...
procedure CalcHS(PMCT: PMCTparameter; hsPsiLight: TPsiLight5; y: Integer);
var itmp, itmp2, ShadowStepCount: Integer;
...
begin
    ...
    begin
      zzVal := ZZ;
      if zzVal < 0 then zzVal := 0; // negative ZZ clip
      itmp := Round(8388352 - ZcMul * (Sqrt(zzVal * Zcorr + 1) - 1));
...
procedure DelayCalcPart(ThreadCount: Integer; PCTS: TPCalcThreadStats);
{$IF DEFINED(WINDOWS) AND NOT DEFINED(MB3D_HEADLESS)}
var Tick: Double;
    Event: THandle;
