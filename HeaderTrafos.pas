...
uses Math,
     {$IFDEF MB3D_HEADLESS}MB3DHeadlessUtils,{$ELSE}DivUtils,{$ENDIF}
     formulas,
     {$IFDEF MB3D_HEADLESS}MB3DHeadlessCustomFormulas,{$ELSE}CustomFormulas,{$ENDIF}
     SysUtils,
     {$IFNDEF MB3D_HEADLESS}LightAdjust,{$ENDIF}
     {$IFDEF LCL}Mand,{$ENDIF}
     {$IFNDEF MB3D_HEADLESS}Navigator,
     Animation, Interpolation, PaintThread, DOF, Tiling,
     FileHandling, MB3DMaps,{$ENDIF}
     Types, Calc
     {$IFDEF MB3D_HEADLESS}, MB3DFormulaThunks{$ENDIF}{, OTrapDEcalc};

procedure ReportMCT(const MessageText: string);
begin
  {$IFDEF LCL}
  Mand3DForm.OutMessage(MessageText);
  {$ENDIF}
end;
...
begin
    ...
    begin
      ...
      begin
        DEoption := -1;
        ReportMCT('Error, mixing of dIFS with other formulas will not work!');
        Break;
      end
      else
      case DEoption of
-1,21,22: DEoption := de;
    ...
    begin
      ...
      begin
        DEoption := -1;
        ReportMCT('Error, no dIFS shape formula selected.');
      end
      else DEoption := 20;
    end;
  ...
  //    Result.bDoubleCheck := (iOptions and 2) > 0;
      ...
      begin
        ...
        if Result.DEoption in [20..22] then
          ReportMCT('Error, dIFS do not work in interpolation hybrid.')
     ...
     //   if (Result.DEoption <> 20) and bDisableADE then Result.IsCustomDE := False;
       ...
       //   if (Result.DEoption2 <> 20) and bDisableADE then Result.IsCustomDE2 := False;
          ...
          begin
            ...
            begin
              ReportMCT('Error, second hybrid type must be dIFS.');
      ...
      begin
        ...
        begin
          bMCTisValid := False;
          ReportMCT('Error, CPU must support SSE2 for dIFS.');
          Exit;
        end;
        if bMCTisValid then
        begin
          ...
          begin
            ...
            begin
              ...
              if calcHybridCustoms[x].bCPmemReserved then
                fHybrid[x] := BindHeadlessFormulaThunk(x,
                  calcHybridCustoms[x].pCodePointer)
              else
   ...
   {    //   if OTrapMode then
          ...
          begin
            ...
            try
              ...
              if bGetMCTPverbose then ReportMCT('Error in calcmap init.');
            end;
            if bGetMCTPverbose then ReportMCT('Parameters ok.');
          end;
        end;
        if not bMCTisValid then ReportMCT('Error, formula option is not valid.');
      end;
    end;
end;
...
begin
  ...
  for i := 0 to 5 do
    ...
    begin
      MCTpars.HSvecs[i] := NormaliseSVectorTo(-MCTpars.StepWidth,
        Lvals.PLValigned.LN[i]);
      RotateVectorReverse(@MCTpars.HSvecs[i], @M);
    end
    else
      MCTpars.HSvecs[i] := AddSVec2Vec3(@Lvals.PLValigned.LN[i],
        @MCTpars.Xmit);
end;

procedure CalcPosLightsRelPos(Header: TPMandHeader10; Lvals: TPLightVals);
begin
  { Positional-light screen picking belongs to the interactive paint path. }
end;

procedure CalcSCstartAndSCmul(Header: TPMandHeader10; var sCStart, sCmul: Single;
  bIsInside: LongBool);
var dTmp: Double;
begin
  with Header.Light do
    ...
    begin
      ...
      sCmul := (Sqr(Sqr((TBoptions shr 7) and $7F)) - dTmp) *
        0.5 * 0.00031605;
    ...
    begin
      ...
      sCmul := Sqr((TBpos[10] + 30) * 0.01111111111111111) * 32767 -
        10900 - sCStart;
      if (TBoptions and $10000) > 0 then
      begin
        dTmp := sCStart + sCmul * (Integer(FineColAdj2) - 30) / 60;
        sCStart := sCStart + sCmul * (Integer(FineColAdj1) - 30) / 60;
        sCmul := dTmp - sCStart;
      end;
      if Abs(sCmul) > s0001 then sCmul := 2 / sCmul
      else if sCmul < 0 then sCmul := -2000 else sCmul := 2000;
    end;
end;
...
procedure MakeLightValsFromHeaderLightNavi(Header: TPMandHeader10;
  Lvals: TPLightValsNavi; ImScale: Double);
begin
  FillChar(Lvals^, SizeOf(Lvals^), 0);
end;
{$ELSE}
